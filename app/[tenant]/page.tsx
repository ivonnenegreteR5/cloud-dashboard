...
export default function DashboardHome() {
  ...
  // tenant del contexto (URL/routing). En tu caso ya lo estás usando para data.
  const tenantFromContext = useTenant() as string | undefined;

  // ✅ YA NO usamos "demo" como fallback. Si no hay tenant listo, no hacemos fetch.
  const tenantId = (tenantFromContext || "").trim();
  ...
  useEffect(() => {
    ...
    // ✅ CLAVE: si aún no hay tenant, NO hacemos requests (evita demo→hach)
    if (!tenantId) return;
      ...
      try {
        ...
        try {
          const cfResp = await fetch(
            `/api/cloud/custom-fields?tenantId=${encodeURIComponent(tenantId)}`,
            { headers }
          );
    ...
    load();
  }, [router, tenantId, pageSize, safePage, skip]);
