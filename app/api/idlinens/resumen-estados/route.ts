//app/api/idlinens/resumen-estados/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${BASE_URL}/api/v1/IdLinens/ResumenEstados`, {
      method: "POST",
      headers,
      ...
      return NextResponse.json(
        {
          ...
          message: "Error consultando ResumenEstados",
          details: data ?? text,
        },
        { status: upstream.status }
      );
    ...
    return NextResponse.json(
      {
        status: 500,
        message: "Error en /api/idlinens/resumen-estados",
