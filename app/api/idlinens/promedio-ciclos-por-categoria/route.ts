...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${BASE_URL}/api/v1/IdLinens/PromedioCiclosPorCategoria`, {
      method: "POST",
      headers,
      ...
      return NextResponse.json(
        {
          ...
          message: "Error consultando PromedioCiclosPorCategoria",
          details: data ?? text,
        },
        { status: upstream.status }
      );
    ...
    return NextResponse.json(
      {
        status: 500,
        message: "Error en /api/idlinens/promedio-ciclos-por-categoria",
