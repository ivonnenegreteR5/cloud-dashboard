// app/api/idlinens/inactivos15/resumen-tipos/route.ts
...
function pickJwt(req: Request, body: any) {
  // 1) si llega Authorization desde el browser, úsalo
  const h = req.headers.get("authorization");
  if (h && h.toLowerCase().startsWith("bearer ")) return h;

  // 2) si no, usa el idToken que tú mandas en body desde api.ts
...
export async function POST(req: Request) {
  try {
    ...
    // 🔁 OJO: AJUSTA este path al que tengas publicado en tu gateway/cloud-run
    // (te lo dejo en forma común según tu patrón)
    const upstreamPath = "/api/v1/IdLinens/Inactivos15/ResumenTipos";
    ...
    const upstream = await fetch(`${BASE_URL}${upstreamPath}`, {
      ...
      body: JSON.stringify({
        auth: { token: sessionToken }, // ✅ tu backend usa sessionToken
        estado,
      }),
      cache: "no-store",
    });
