///app/api/idlinens/totales-por-tipo/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${BASE_URL}/api/v1/IdLinens/TotalesPorTipo`, {
      method: "POST",
      headers,
      body: JSON.stringify({ auth: { token: sessionToken } }),
      cache: "no-store",
    });
    ...
    if (!upstream.ok) {
      return NextResponse.json(
        { status: upstream.status, message: "Error consultando TotalesPorTipo", details: data ?? text },
    ...
    return NextResponse.json(
      { status: 500, message: "Error en /api/idlinens/totales-por-tipo", details: String(err?.message || err) },
      { status: 500 }
    );
  }
}
